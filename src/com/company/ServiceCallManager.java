...
public class ServiceCallManager {
    ...
    private void addWaterHeaterServiceCall(){
        //Getting data about water heater and creating water heater object to add to ServiceCalls
        String address = Input.getStringInput("Enter address of water heater");
        String problem = Input.getStringInput("Enter description of problem");
        String ageString = Input.getStringInput("Enter age of water heater, in years");

        WaterHeater wh = new WaterHeater(address, problem, new Date(), ageString);
        todayServiceCalls.add(wh);
        System.out.println("Added the following water heater list of calls:\n" + wh);
